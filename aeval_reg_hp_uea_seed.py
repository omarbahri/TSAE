...
from sktime_convert import load_from_tsfile_to_dataframe, from_nested_to_3d_numpy
...
for name in [name]:  
    # try:
        ...
        X_train, y_train = load_from_tsfile_to_dataframe(
            os.path.abspath(os.path.join('..', '..', '..', 'st_rules', 'data', 'UEA_26', name, name + "_TRAIN.ts"))
        )
        X_test, y_test = load_from_tsfile_to_dataframe(
                os.path.abspath(os.path.join('..', '..', '..', 'st_rules', 'data', 'UEA_26', name, name + "_TEST.ts"))
            )
        ...
        if scale==1:
            ...
            output_csv_path = os.path.join(results_path, 'orig_uea_' + str(seed) +\
            ...
            for dim in range(X_train.shape[2]):
                ...
                if min_val >= 0:
                    ...
                    X_test[:,:,dim] = (X_test - min_val) / (max_val - min_val)
